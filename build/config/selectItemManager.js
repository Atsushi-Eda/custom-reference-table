export default class selectItemManager {
    static createItems(unFormattedItems, type = null) {
        ...
        if (Array.isArray(unFormattedItems)) {
            items.push(...unFormattedItems.map(unFormattedItem => (type === 'fields' ? {
    ...
    static getValue({ unFormattedItems, value = '', type = null }) {
        return (Array.isArray(unFormattedItems) && unFormattedItems.find(unFormattedItem => (type === 'fields' ? unFormattedItem.code === value : unFormattedItem === value))) ? value : '';
    }
