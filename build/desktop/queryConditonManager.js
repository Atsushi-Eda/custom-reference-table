export default class queryCondition {
    static create(conditions, record) {
        if (!Array.isArray(conditions))
            return '';
        return conditions.filter(condition => (condition.targetField && condition.operator && condition.selfField)).map(condition => this.createUnit(condition, record)).join(' and ');
    }
